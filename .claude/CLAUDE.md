...
`/commit-push` — stages all changes, scans for secrets, proposes a commit message, and optionally pushes (defined in `.claude/skills/commit-push/SKILL.md`).
`/skill-creator` — guide for creating, modifying, improving, and measuring skill performance (defined in `.claude/skills/skill-creator/SKILL.md`).
